...
try:
    ...
    from src.identification.data_utils.documents import Document
...
class PICKDataset(Dataset):
    ...
    def __init__(self, entities_list: List,
        ...
        if self.training:  # used for train and validation mode
            ...
            self.labels_folder: Path = Path(labels_folder)
    ...
    def get_image_file(self, basename):
        ...
        self._image_ext = '.png'
        path = self.images_folder.joinpath(basename + self._image_ext)
        if path.exists():
            return path
        return self.images_folder.joinpath(basename + '.jpg')
    ...
    def get_label_file(self, basename):
        """
        Return the complete name (fill the extension) from the basename.
        """
        if self._ann_ext is None:
            filename = list(self.labels_folder.glob(f'**/{basename}.*'))[0]
            self._ann_ext = os.path.splitext(filename)[1]

        return self.labels_folder.joinpath(basename + self._ann_ext)
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            label_file = self.get_label_file(Path(self.files_list[index]).stem)
        ...
        try:
            ...
            if self.training:
                ...
                document = documents.Document(boxes_and_transcripts_file, image_file, label_file,entities_list=self._entities_list,
                                              resized_image_size=self.resized_image_size,
...
class TestingDataset(Dataset):
    ...
    def __init__(self, image,
        ...
        self.ignore_error = ignore_error
        self.image = image
        self.boxes_and_transcripts_data= boxes_and_transcripts_data
    ...
    @overrides
    def __getitem__(self, index):
        document = documents.TestingDocument(image=self.image,
                                             boxes_and_transcripts_data=self.boxes_and_transcripts_data,
                                             image_index=index)
        return document
...
class BatchCollateFn(object):
    ...
    def __call__(self, batch_list: List[Document]):
        ...
        if self.training:
            ...
            # label_file = [x.label for x in batch_list]
